...
export const GoogleAuthCallbackURL : string = "http://localhost:3000/auth/callback";
export const FBAppID : string = "1670075207224447";

export const startSession = async () => {
    const res = await fetch(`${serverUri}:${serverPort}/start-session`, { method: "POST", credentials: "include", headers: { "Content-type": "application/json" }});
...
export const isLoggedIn = async () => {
    ...
    const res = await fetch(`${serverUri}:${serverPort}/is-logged-in`, { method: "POST", credentials: "include", headers: { "Content-type": "application/json" }});
    const response = await res.json();
    console.log(response.data.isLoggedIn)////
    return response.data.isLoggedIn;
}
...
export const LogOut = async () => {
    ...
    const res = await fetch(`${serverUri}:${serverPort}/logout`, {
        credentials: "include"
    });
...
export const authUserWithFb = async (fbId : string) => {
    ...
    const response = await fetch(`${serverUri}:${serverPort}/auth-fb`, {
        method: "POST",
...
export const registerUser = async (formData: any) => {
    ...
    const response = await fetch(`${serverUri}:${serverPort}/sign-up`, {
        method: "POST",
...
export const authenticateUser = async (formData: any) => {
    const jsonData = Object.fromEntries(formData.entries());
    const response = await fetch(`${serverUri}:${serverPort}/login`, {
        method: "POST",
        body: JSON.stringify(jsonData),
        credentials: "include",
        headers: {
...
export const sendChatSetupData = async (formData: any) => {
    ...
    const result = await fetch(`${serverUri}:${serverPort}/reg-chat-data`, {
        method: "POST",
        body: formData,
        credentials: 'include',
