...
import { getColorFromName, getInitials, serverPort, serverUri } from "../constant";
  ...
  const [channel, setChannel] = useState<string | undefined>("c")
      ...
      useEffect(() => {
        ...
        const getChats = async () => {
          try {
            const response = await fetch(`${serverUri}:${serverPort}/user-get-messages`, {
              method: "POST",
  ...
  if(emojiContainer.innerHTML === ""){
   ...
   const ai = () => {
    ...
    const sendToAi = async (event: React.MouseEvent<HTMLDivElement>) => {
      ...
      const result = await fetch(`${serverUri}:${serverPort}/ai-assist-message`, {
        method: "POST",
    ...
    const sendMessage = async () => {
      ...
      fetch(`${serverUri}:${serverPort}/user-send-message`, {
          method: "POST",
          body: formData,
          credentials: "include",
      })
